...
(function (global, factory) {
    ...
    var ckplayerJS = 'static/myVedio_js/ckplayer/ckplayer.js';
    var ckplayerSWF = 'static/myVedio_js/ckplayer/ckplayer.swf';
    var m3u8SWF = 'static/myVedio_js/ckplayer/m3u8.swf';
    var flv_js = '/sdk/myVedio_js//flv.min.js';
    var hlsJS = 'static/myVedio_js/hls.min.js';
    var mpegJS = 'static/myVedio_js/jsmpeg.min.js';
    var wav = Domain + '/sdk/myVedio_js/wav-audio-encoder.js';
    ...
    var EZUIPlayer = function (playParams) {
        ...
        // // 获取appKey
        // request(domain + 'static/myVedio_jssdk/ezopen/getAppInfo?uuid=' + playUid + '&accessToken=' + accessToken + "&deviceSerial=" + deviceSerial + "&channelNo=1",
        //   'GET',
        //   '',
        //   '',
        //   appInfoSuccess,
    ...
    EZUIPlayer.prototype.initHLS = function (hlsURL) {
        console.log('初始化hls.js')
        var me = this;
        var hls = new Hls({defaultAudioCodec: 'mp4a.40.2'}); // 萤石设备默认使用 AAC LC 音频编码
        hls.loadSource(hlsURL);
        hls.attachMedia(this.video);
        hls.on(Hls.Events.MANIFEST_PARSED, function () {
            console.log('isdestroyed：',me.opt.isdestroyed)
            if(!me.opt.isdestroyed){
                if (me.opt.autoplay) {
                    me.video.play();
                }
                me.initVideoEvent();
            ...
            if (data.fatal) {
                switch (data.type) {
                    case Hls.ErrorTypes.NETWORK_ERROR:
                        ...
                        console.log("NETWORK_ERROR, try to recover");
                        hls.startLoad();
                        // hls.destroy();
                        break;
                    case Hls.ErrorTypes.MEDIA_ERROR:
                        console.log("MEDIA_ERROR, try to recover");
    ...
    EZUIPlayer.prototype.initDecoder = function (playParams) {
        ...
        function initDecoder(resolve, reject) {
            var jsPluginPath = playParams.decoderPath + 'static/myVedio_js/jsPlugin-1.2.0.min.js';
            ...
            addJs(jsPluginPath, function () {
                ...
                _this.jSPlugin = new JSPlugin({
                    ...
                    szBasePath: playParams.decoderPath + 'static/myVedio_js',
                    oStyle: {
                        border: "none",
                        background: "#000000"
                    }
            ...
            if (!window.localStorage) {
                request(
                    playParams.decoderPath + "static/myVedio_js/errorCode.json",
                    "get",
                    {
                        language: 1,
    ...
    EZUIPlayer.prototype.stop = function (i, unDestory) {
        ...
        this.opt.isdestroyed=true
        if (!!window['CKobject']) {
            //CKobject.getObjectById(this.flashId).destroy();
            this.video.src = ""
            delete window['CKobject']   //自己改的，有问题改回来;
            // this.video.remove();
        }
    if (!!this.video) {
            if (!!this.hls) {   // hls停止依赖this.hls
                // 通过暂停停止播放
                console.log('通过暂停停止播放')
                setTimeout(()=>{
                    this.video.pause();
                    this.video.src = ""
                    // 停止取流
                    // console.log(this.hls)
                    this.hls.destroy();           //自己改的，有问题改回来
                    this.hls=null
                    // this.hls.stopLoad();            无效
                },10)
            } else if (!!this.flv) {
                this.flv.pause();
                this.flv.unload();
    ...
    // 视频截图
    EZUIPlayer.prototype.capturePicture = function (iWind, pictureName, callback) {
        if (!!this.jSPlugin) {
            return this.jSPlugin.JS_CapturePicture(iWind, pictureName, 'JPEG', callback);
        } else {
            throw new Error("Method  not support");
        }
    }
